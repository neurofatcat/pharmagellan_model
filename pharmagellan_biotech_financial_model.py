    ...
    if ticker_symbol:
        ...
        for i in range(num_assets):
            ...
            phase = st.selectbox(
                ...
                options=list(PHASE_PROBABILITIES.keys()),
                key=f"phase_{i}",
            ...
            market_penetration_rate = st.slider(
                ...
                help="The percentage of the eligible population expected to use this treatment."
            )

            price_per_patient = st.number_input(
                f"Price per Patient for Asset {i+1} (in thousands USD):",
                min_value=0.0,
                value=0.0,
                key=f"price_{i}",
                help="The cost of the treatment per patient in thousands of USD."
            ) * 1e3

            delay_years = st.slider(
                f"Years with No Cash Flow for Asset {i+1}:",
                min_value=0,
                max_value=20,
                value=0,
                key=f"delay_{i}",
                help="The number of years before the asset generates revenue."
            )

            initial_revenue = st.number_input(
                f"Initial Revenue for Asset {i+1} (in billions USD):",
                min_value=0.0,
                value=0.0,
                key=f"revenue_{i}",
                help="The expected initial revenue generated by this asset in billions of USD."
            ) * 1e9

            growth_rate = st.slider(
                f"Annual Revenue Growth for Asset {i+1} (%):",
                min_value=0,
                max_value=50,
                value=0,
                key=f"growth_{i}",
                help="The annual growth rate of the revenue for this asset."
            )

            years_to_simulate = st.slider(
                f"Years to Simulate for Asset {i+1}:",
                min_value=0,
                max_value=50,
                value=0,
                key=f"simulate_{i}",
                help="The number of years to include in the revenue simulation for this asset."
            )
